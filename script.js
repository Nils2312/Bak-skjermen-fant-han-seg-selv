...
window.addEventListener("scroll", updateHealth);
...
for (let i = 0; i < totalBalls; i++) {
  ...
  balls.push({ el: ball, x: startX, y: startY });
}
...
function updateBalls() {
  ...
  balls.forEach((ball) => {
    ...
    ball.targetX = startX + (clusterX - startX) * visible;
    ball.targetY = startY + (clusterY - startY) * visible;
  });
